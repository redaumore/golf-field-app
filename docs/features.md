...
| ID | Funcionalidad | Área | Estado | Descripción | Notas |
|---|---|---|---|---|---|
| P-08 | Configuración de "Mi Bolsa" | Perfil del jugador | ✅ Implementado | Panel para gestionar la bolsa: ver, agregar (incluyendo palos personalizados por sigla) y eliminar palos, con reordenamiento por drag-and-drop. La grilla de palo del hoyo filtra según la bolsa. | El palo pasó de enum fijo a `string`. `LostBall` nunca entra a la bolsa pero siempre aparece en el hoyo. Distancias/características físicas personalizadas quedan para P-09. |
